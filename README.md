...
<td>
화면 드래그하면 플레이어가 따라오고, 손을 떼면 그 자리에 정지. 카드를 고른 즉시 발사 패턴이 바뀜. 우하단 스킬버튼 영역은 잘못터치하여 드래그 이동이 안 되도록 방지하여, 위급한 순간에 스킬을 누르려다 플레이어가 끌려가는 사고를 방지.
</td>
</tr>
</table>
...
<br/><br/>
<b>▸ 동작 구성</b><br/>
화면 위쪽에서 등장해 종별로 다른 행동을 한다 — 자폭병/분열병은 화면 중간까지 내려온 후 목표 위치를 플레이어 방향으로 바꾼다, 원거리병은 멈춰서 플레이어 방향으로 탄환 발사, 미니언은 분열 직후 사선으로 튀어나간 뒤 락온. 시간이 흐를수록 등장 빈도와 종류 비율이 다양해지고 HP 도 점차 강해진다.
</td>
</tr>
<tr>
<td valign="top"><b>상호작용 정보</b></td>
...
<td valign="top"><b>UX 진행 방법</b></td>
<td>
공중에서 몰려오는 적을 처치하면 된다. 몬스터가 너무 많을 경우 자폭병과 분열 몬스터는 위치를 유도한 하 도망갈 수 있다.
</td>
</tr>
</table>
...
<br/><br/>
<b>▸ 동작 구성</b><br/>
같은 클래스 1개를 공유하고 인자만 다르게 줘서 만든다. 발사 후 직진하다 화면 밖이면 사라지고, 유도탄은 가장 가까운 적 방향으로 부드럽게 휘어 들어간다. 샷건의 경우 부채꼴 형태로 탄이 퍼져나간다. 레이저만 별도 클래스로 분리되어 직선 영역 안 모든 적에게 매 프레임 데미지를 입힌다.
</td>
</tr>
<tr>
<td valign="top"><b>상호작용 정보</b></td>
...
<td valign="top"><b>UX 진행 방법</b></td>
<td>
발사는 자동이라 사용자는 신경 쓸 필요가 없다. 무기 카드를 고르는 즉시 플레이어가 쏘는 총알이 바뀐다. 샷건의 경우 부채꼴 형태의 공격을 하고 유도탄은 휘어 들어가는 모션과 함께 적을 추적하며, 레이저는 빔이 한 라인의 적에게 연속적인 데미지를 준다.
</td>
</tr>
</table>
...
<td valign="top"><b>UX 진행 방법</b></td>
<td>
사용자가 직접 주우러 다닐 필요가 없어 EXP오브가 자동으로 들어와 전투에 집중할 수 있다. 시간이 지날수록 한 개당 가치가 커지므로 적이 강해지는 것과 보상의 가속도가 균형을 이룬다.
</td>
</tr>
</table>
...
<td valign="top"><b>UX 진행 방법</b></td>
<td>
평소엔 자동 발사로 진행하다가 위급한 순간에 한 번 탭하면 폭발·회복·버프가 즉시 들어간다. 쿨다운 동안엔 원이 채워지면서 다음 사용 가능 시점을 알려 준다.
</td>
</tr>
</table>
...
<br/><br/>
<b>▸ 동작 구성</b><br/>
시트를 애니메이션으로 자동 분할 재생한다. 한 클래스가 3개의 모드 (1 회성·지속형·추종형) 를 모두 처리하며, 광폭화만 지속형으로 7 초간 플레이어 머리 위를 따라다니는 오라가 되고 나머지는 한 번 재생 후 소멸한다.
</td>
</tr>
<tr>
<td valign="top"><b>상호작용 정보</b></td>
...
**핵심 코드 — 실제 점수와 표시 점수 분리**

  ```kotlin
  private var displayScore = 0  // 화면에 그릴 값 (실제 점수와 별개)

  override fun update(gctx: GameContext) {
      val target = scene.score          // 실제 누적 점수
  ...
  `MainScene.score` (실제 점수) 와 `displayScore` (화면에 그릴 점수) 를 **별개의 변수로 분리**해 두 값의 차이를 매
  프레임 좁혀 간다. 차이가 9 이하면 ±1, 그보다 크면 `diff / 10` 씩 따라잡는다. 이렇게 실제 값과 표시 값을 분리하여 부드럽게 점수가 올라가는 애니메이션을 구현한다.
...
<td valign="top"><b>UX 진행 방법</b></td>
<td>
다음 보스 입장 타이밍을 인지하는 핵심 지표. 시간이 보일 때는 카운트다운, "BOSS STAGE" 에서는 보스 스테이지임을 알리는 hud 역할을 한다.
</td>
</tr>
</table>
...
<br/><br/>
<b>▸ 동작 구성</b><br/>
플레이어의 현재 레벨과 다음 레벨까지의 EXP 진행도를 그대로 표시. 자체 게이지 없이 숫자로만 표시한다.
</td>
</tr>
<tr>
<td valign="top"><b>상호작용 정보</b></td>
...
<td valign="top"><b>UX 진행 방법</b></td>
<td>
디버그/시연용 라벨 — 카드 보상이 능력치에 어떻게 반영됐는지 숫자로 확인 가능. 광폭화 동안 빨간색으로 바뀌면서 버프가 적용된 실제 수치를 보여 주어 확인하기 쉽게 한다.
</td>
</tr>
</table>
...
<td valign="top"><b>상호작용 정보</b></td>
<td>
적이 만들어질 때마다 풀 팩토리에서 꺼내 재사용한다. 게임이 진행될수록 스폰 간격이 짧아지도록 한다.
</td>
</tr>
<tr>
<td valign="top"><b>UX 진행 방법</b></td>
<td>
0~5 초 자폭병만 등장, 그 후 원거리·분열형이 차례로 섞여 들어오며, 시간에 따라 몬스터의 스폰속도가 늘어나 난이도를 증가시킨다.
</td>
</tr>
</table>
...
<td valign="top"><b>UX 진행 방법</b></td>
<td>
데미지 popup 은 기본적으로는 흰색, 치명타 시 노란색 + 더 큰 글자로 표시하여 데미지가 체감되게 한다.
</td>
</tr>
</table>
